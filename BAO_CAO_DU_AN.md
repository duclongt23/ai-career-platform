...
- **Admin:** quản lý nghề nghiệp, Core Quiz, yếu tố nghề nghiệp và mapping điểm.
...
Lịch sử Career Explore Chat hiện lưu ở `localStorage` theo từng nghề, chưa đồng bộ backend.
...
Lưu thông tin tài khoản, mật khẩu đã hash và role.
...
`DEEPSEEK_MODEL` và `TAVILY_API_KEY` có thể bỏ qua tùy cấu hình. Nếu thiếu Tavily, Career Explore Chat vẫn trả lời nhưng không có dữ liệu web cập nhật.
...
- Backend register hiện nhận `role` từ request; trước production cần khóa đăng ký công khai về `student` và tạo admin bằng cơ chế riêng.
...
- Career Explore Chat lưu lịch sử ở `localStorage`, chưa đồng bộ giữa thiết bị.
...
- Thêm refresh token, rate limit, validation schema và cơ chế tạo admin riêng.
...
- Lưu Career Explore Chat ở backend và bổ sung rate limit cho endpoint hội thoại.
